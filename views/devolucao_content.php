...
<div class="form-wrapper">
  ...
  <div class="top-buttons">
      <button type="button" class="botao-form" onclick="imprimirComprovante()">Imprimir Comprovante</button>
  </div>
<form action="salvar_devolucao.php" method="POST">
    <label for="id_objeto">ID do Objeto:</label>
<?php
  $id_objeto = isset($_GET['id']) ? htmlspecialchars($_GET['id']) : '';
?>
<input type="text" id="id_objeto" name="id_objeto" value="<?= $id_objeto ?>" <?= $id_objeto ? 'readonly' : '' ?> required />
